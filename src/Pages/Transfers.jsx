...
export default function TransferManager() {
    ...
    const filteredTransfers = transfers.filter(tr => {
        return (filterBase === "All" || tr.from === filterBase || tr.to === filterBase) &&
        ...
        <div className="max-w-[1100px] mx-auto mt-10 px-3 font-sans">
            <div className="flex justify-between items-center mb-6 flex-wrap gap-6">
                ...
                <div className="flex flex-wrap gap-3 items-center">
                    ...
                    >
                        <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24"><path d="M12 4v16m8-8H4" /></svg>
                        New Transfer
                    </button>
                </div>
            </div>
                    ...
                    <form onSubmit={handleSubmit} className="space-y-6">
                        <div>
                            ...
                            >
                                ...
                                {assetOptionsSelect.map((opt, idx) => (
                            ...
                            pagedTransfers.map((tr, i) => (
                                <tr key={i} className="hover:bg-[#EAF4FF] even:bg-gray-50">
                                    ...
                                    <td className="px-4 py-4 text-right whitespace-nowrap">
                                        <button
                                            onClick={() => alert(`View transfer: ${tr.asset}`)}
                                            className="text-[#198CF7] font-semibold hover:underline cursor-pointer"
                                        >
                                            View
                                        </button>
                                    </td>
            ...
            <div className="flex flex-wrap justify-end items-center gap-4 p-4 text-sm">
                ...
                <button
                    ...
                    className={`min-w-[30px] min-h-[30px] rounded font-semibold border ${page <= 1 ? "border-gray-300 text-gray-400 cursor-not-allowed" : "border-[#198CF7] text-[#198CF7] hover:bg-[#198CF7] hover:text-white"}`}
                >
                    &lt;
                </button>
                {Array.from({ length: totalPages }, (_, i) => (
                    <button
                        key={i}
                        onClick={() => setPage(i + 1)}
                        className={`min-w-[30px] min-h-[30px] rounded font-semibold border ${page === i + 1 ? "bg-[#198CF7] text-white border-[#198CF7]" : "border-[#198CF7] text-[#198CF7] hover:bg-[#198CF7] hover:text-white"}`}
                    >
                        {i + 1}
                    </button>
                ))}
                <button
                    ...
                    onClick={() => setPage(page + 1)}
                    className={`min-w-[30px] min-h-[30px] rounded font-semibold border ${page >= totalPages ? "border-gray-300 text-gray-400 cursor-not-allowed" : "border-[#198CF7] text-[#198CF7] hover:bg-[#198CF7] hover:text-white"}`}
                >
                    &gt;
                </button>
            </div>
