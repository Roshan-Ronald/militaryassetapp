...
async function createNotification(notification) {
  ...
  const newNotification = { id: Date.now(), ...notification, date: new Date().toLocaleString() };
...
export default function UserProfile() {
  ...
  const [profile, setProfile] = useState({ fullName: "", username: "", email: "", role: "" });
  ...
  const handlePasswordChange = async (e) => {
    ...
    if (newPassword !== confirmPassword) {
      ...
      await createNotification({ message: "Password change failed: mismatch", type: "error" });
      return;
    }
    toast.info("Password change functionality is not implemented");
    await createNotification({ message: "Password change attempted", type: "info" });
  };
    ...
    <div className="min-h-screen bg-gradient-to-tr from-white via-gray-50 to-blue-50 p-4 sm:p-8 lg:p-12 font-sans text-gray-900">
      ...
      <div className="max-w-4xl mx-auto">
        <header className="mb-8">
          <h1 className="text-3xl sm:text-4xl font-extrabold tracking-tight mb-6 drop-shadow-md">User Profile</h1>
          <nav className="inline-flex bg-white shadow rounded-lg overflow-hidden">
            <button
              className={`w-full sm:w-auto px-6 py-3 font-semibold tracking-wide transition-colors duration-300 focus:outline-none ${page === "profile" ? "bg-blue-600 text-white shadow-lg" : "text-gray-600 hover:bg-blue-100"
                }`}
            ...
            <button
              className={`w-full sm:w-auto px-6 py-3 font-semibold tracking-wide transition-colors duration-300 focus:outline-none ${page === "changePassword" ? "bg-blue-600 text-white shadow-lg" : "text-gray-600 hover:bg-blue-100"
                }`}
        ...
        {page === "profile" && (
          <section className="bg-white rounded-3xl p-8 sm:p-10 shadow-xl ring-1 ring-gray-200">
            <h2 className="text-2xl sm:text-3xl font-bold mb-6 text-gray-900 tracking-wide">User Information</h2>
            <p className="text-gray-600 mb-10 max-w-xl">View and manage your personal details and application settings.</p>
            <dl className="grid grid-cols-1 sm:grid-cols-2 gap-8 text-gray-800">
              ...
              <div>
                ...
                <dd className="flex items-center space-x-6">
                  <span
                    className={`inline-block px-5 py-2 rounded-full font-semibold shadow ${accountStatus === "Active"
        ...
        {page === "changePassword" && (
          <section className="bg-white rounded-3xl p-8 sm:p-10 shadow-xl ring-1 ring-gray-200 max-w-lg mx-auto mt-8">
            ...
            <form onSubmit={handlePasswordChange} className="space-y-6">
              <div>
                <label htmlFor="currentPassword" className="block text-gray-700 mb-2 font-semibold">Current Password</label>
                <input
                  id="currentPassword"
                  type="password"
              ...
              <div>
                <label htmlFor="newPassword" className="block text-gray-700 mb-2 font-semibold">New Password</label>
                <input
                  id="newPassword"
                  type="password"
                  value={newPassword}
              ...
              <div>
                <label htmlFor="confirmPassword" className="block text-gray-700 mb-2 font-semibold">Confirm New Password</label>
                <input
                  id="confirmPassword"
                  type="password"
