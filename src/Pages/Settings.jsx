...
import {  getSettings,  saveSettings,  getAssetTypes,  saveAssetTypes,  getBases,  saveBases,} from '../Api';
  ...
  return (
    <div className="min-h-screen py-8 px-4 sm:px-6 lg:px-8 font-sans">
      <div className="max-w-7xl mx-auto">
        ...
        <div>
          {activeTab === 0 && (
            <form className="bg-white shadow p-6 rounded-lg max-w-4xl mx-auto" onSubmit={handleSettingsSave}>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 mb-6">
                ...
                <div>
                  <label className="block mb-1 font-medium text-gray-700">Default Currency</label>
                  <select
                    className="w-full border border-gray-300 rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500"
                ...
                <div>
                  <label className="block mb-1 font-medium text-gray-700">Date Format</label>
                  <select
                    className="w-full border border-gray-300 rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500"
                ...
                <div>
                  <label className="block mb-1 font-medium text-gray-700">Time Format</label>
                  <select
                    className="w-full border border-gray-300 rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500"
                ...
                <div className="flex items-center mt-6 col-span-full">
                  ...
                  />
                  <label className="text-gray-700 font-medium select-none">Enable email notifications</label>
                </div>
              </div>
              <button
                type="submit"
                className="ml-auto block bg-blue-600 text-white px-8 py-3 rounded hover:bg-blue-700 transition"
              >
                Save
              </button>
            </form>
          ...
          {activeTab === 1 && (
            <div className="bg-white shadow p-6 rounded-lg max-w-4xl mx-auto">
              ...
              <div className="flex gap-3 mb-4">
                ...
                />
                <button className="bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700" onClick={handleAssetAdd}>Add</button>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                {assetTypes.map(asset => (
                  <div key={asset} className="flex justify-between items-center border rounded px-4 py-2">
                    <span>{asset}</span>
                    <button className="text-red-500 hover:underline" onClick={() => handleAssetRemove(asset)}>Remove</button>
                  </div>
                ))}
              </div>
            </div>
          ...
          {activeTab === 2 && (
            <div className="bg-white shadow p-6 rounded-lg max-w-4xl mx-auto">
              ...
              <div className="flex gap-3 mb-4">
                ...
                />
                <button className="bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700" onClick={handleBaseAdd}>Add</button>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                {bases.map(base => (
                  <div key={base} className="flex justify-between items-center border rounded px-4 py-2">
                    <span>{base}</span>
                    <button className="text-red-500 hover:underline" onClick={() => handleBaseRemove(base)}>Remove</button>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="bg-white shadow p-6 rounded-lg max-w-3xl mx-auto">
              ...
              <div className="flex items-center justify-between mb-6 border rounded p-4 bg-gray-50">
                <div>
                  ...
                  <p className="text-gray-600 text-sm">When enabled, only administrators can access the system</p>
                </div>
                <button
                  className={`px-6 py-2 rounded font-semibold ${maintenanceMode ? 'bg-gray-300 text-gray-800' : 'bg-green-600 text-white hover:bg-green-700'}`}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 mb-6">
                <div>
                  ...
                  <p className="text-gray-900 font-semibold">{settings.version}</p>
                </div>
                <div>
                  <p className="text-gray-600 text-sm mb-1">Database Status</p>
                  <p className="inline-block rounded-full px-3 py-1 bg-red-100 text-red-700 font-semibold text-sm">{settings.databaseStatus}</p>
                </div>
                <div>
                  <p className="text-gray-600 text-sm mb-1">Last Updated</p>
                  <p className="text-gray-900 font-semibold">{settings.lastUpdated}</p>
                </div>
                <div>
                  <p className="text-gray-600 text-sm mb-1">API Status</p>
                  <p className="inline-block rounded-full px-3 py-1 bg-red-100 text-red-700 font-semibold text-sm">{settings.apiStatus}</p>
                </div>
              </div>
              <button className="ml-auto block bg-blue-600 text-white px-8 py-3 rounded hover:bg-blue-700 transition" onClick={handleSettingsSave}>Save</button>
            </div>
          )}
        </div>
      </div>
